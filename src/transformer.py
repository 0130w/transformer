...
class PositionalEmbedding(nn.Module):
  def __init__(self, d_model, max_len) -> None:
    ...
    pe[:, 1::2] = torch.cos(angles[:, :d_model // 2]) # compatible with d_model is odd

    self.register_buffer("pe", pe.unsqueeze(0)) # [max_len, d_model] -> [1, max_len, d_model]
...
class MultiHeadAttention(nn.Module):
  ...
  def forward(self, x): # [batch_size, seq_len, d_model]
    ...
    k = self.k_proj(x)
    v = self.v_proj(x)
    q = torch.reshape(q, (batch_size, seq_len, self.head_num, self.head_dim)).transpose(1, 2) # [batch_size, head_num, seq_len, head_dim]
    k = torch.reshape(k, (batch_size, seq_len, self.head_num, self.head_dim)).transpose(1, 2)
    v = torch.reshape(v, (batch_size, seq_len, self.head_num, self.head_dim)).transpose(1, 2)
    scores = q @ k.transpose(-1, -2)  # [batch_size, head_num, seq_len, seq_len]
    scores = scores / math.sqrt(self.head_dim)
    weights = torch.softmax(scores, -1)
    out = (weights @ v).transpose(1, 2).reshape(batch_size, seq_len, self.d_model) # [batch_size, seq_len, d_model]
